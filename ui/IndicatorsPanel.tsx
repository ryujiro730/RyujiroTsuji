...
type Props = {
  indicators: {[name: string]: any}
  onChange: (name: string, params: any) => void
...
export default function IndicatorsPanel({ selectedIndicator, onChange }: { selectedIndicator: string, onChange: (settings: any) => void }) {
  ...
  def?.params?.forEach((param) => { initial[param.name] = param.default })

  const [settings, setSettings] = useState<Record<string, any>>({ ...initial, buy: 0, sell: 0 })

  // 選択インジケータ切り替え時に初期値リセット
  useEffect(() => {
    setSettings({ ...initial, buy: 0, sell: 0 })
  }, [selectedIndicator])
  ...
  // 親へのonChangeは「保存して閉じる」で呼ぶほうが確実
  // もしリアルタイム反映したいなら↓を使う
  // useEffect(() => {
  //   onChange({ [selectedIndicator]: settings })
  // }, [settings])
